...
class JackTokenizer:
    ...
    def remove_comments(self):
        for item in JackTokenizer.MULTI_LINE_COMMENTS:
            self.file_text = re.sub('^[ ]*({0}.*?{1})'.format(item[0], item[1]),
                                '', self.file_text, flags=re.DOTALL | re.MULTILINE)
        for item in JackTokenizer.SINGLE_LINE_COMMENTS:
            self.file_text = re.sub('^([^\n\"]*?|.*\".*\".*)(//.*)'.format(item[0]), r'\1',
                                    self.file_text, flags=re.MULTILINE)

    def has_more_tokens(self):
        rest_of_file = self.file_text[self.char_counter:]
    ...
    def advance(self):
        while True:
            if self.char_counter == len(self.file_text):
                return False

            current_char = self.file_text[self.char_counter]
